...
export default function Cart() {
  ...
  useEffect(() => {
    if (customer) {
      fetch(`https://server3-route-omkar-07-dev.apps.sandbox-m4.g2pi.p1.openshiftapps.com/api/get/${customer}`)
        .then((res) => res.json())
        .then((result) => {
          setData(result);
  ...
  const handleDelete = (cartId) => {
    fetch(`https://server3-route-omkar-07-dev.apps.sandbox-m4.g2pi.p1.openshiftapps.com/api/deleteCart/${cartId}`, {
      method: 'DELETE',
    })
      .then((res) => res.json())
      .then((data) => {
