...
export default function DeleteSelected() {
  ...
  useEffect(() => {
    const apiUrl = `https://server3-route-omkar-07-dev.apps.sandbox-m4.g2pi.p1.openshiftapps.com/admins/${url}/${code}`;
    console.log(apiUrl);
    fetch(apiUrl)
  ...
  const handleDelete = async () => {
    ...
    const res = await fetch(`https://server3-route-omkar-07-dev.apps.sandbox-m4.g2pi.p1.openshiftapps.com/admins/${url}/${code}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (res.ok) {
